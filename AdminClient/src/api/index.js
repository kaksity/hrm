...
export default new function API() {
    ...
    this.PostNoticeBoard = params => request.post('/notice-boards', params);
    this.GetNotices = () => request.get('/notice-boards');
    this.GetSingleNotice = id => request.get(`/notice-boards/${id}`);
    this.DeleteNotice = id => request.delete(`/notice-boards/${id}`);
    this.PutNoticeBoard = (id, params) => request.put(`/notice-boards/${id}`, params);
